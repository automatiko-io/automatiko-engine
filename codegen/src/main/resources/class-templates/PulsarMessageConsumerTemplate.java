...
public class $Type$MessageConsumer {
	...
	public CompletionStage<Void> consume(Message<?> msg) {
        ...
        try {
            io.smallrye.reactive.messaging.pulsar.PulsarMessage<?> record = (io.smallrye.reactive.messaging.pulsar.PulsarMessage<?>) msg;
	...
	private String correlation($DataType$ eventData, Message<?> msg) {
        ...
        if (correlation == null && ((io.smallrye.reactive.messaging.pulsar.PulsarMessage<?>) msg).getKey() != null) {
            correlation = ((io.smallrye.reactive.messaging.pulsar.PulsarMessage<?>) msg).getKey().toString();
        }
        return correlation;
	}
    ...
    protected String header(Message<?> message, String name) {
        ...
        io.smallrye.reactive.messaging.pulsar.PulsarIncomingMessageMetadata metadata = message.getMetadata(io.smallrye.reactive.messaging.pulsar.PulsarIncomingMessageMetadata.class).orElse(null);
        if (metadata == null) {
            return null;
        }
    ...
    protected void cloudEventsExtensions(Message<?> message, $DataEventType$ event) {
        ...
        for (Entry<String, String> prop : properties.entrySet()) {
            if (prop.getKey().startsWith("ce_"))
                event.addExtension(prop.getKey().replaceFirst("ce_", ""), prop.getValue());
        }
    }
}