...
public class $Type$MessageConsumer {
	...
	protected String correlationEvent(io.automatiko.engine.api.event.AbstractDataEvent<?> eventData, Message<byte[]> message) {
   ...
   protected String contentType(Message<byte[]> message) {

       io.smallrye.reactive.messaging.rabbitmq.IncomingRabbitMQMetadata metadata = message.getMetadata(io.smallrye.reactive.messaging.rabbitmq.IncomingRabbitMQMetadata.class).orElse(null);
        if (metadata == null) {
            return "application/json; charset=UTF-8";
        }
        return metadata.getContentType().orElse("application/json; charset=UTF-8");        
    }
	
   protected String appProperty(Message<byte[]> message, String name) {

       io.smallrye.reactive.messaging.rabbitmq.IncomingRabbitMQMetadata metadata = message.getMetadata(io.smallrye.reactive.messaging.rabbitmq.IncomingRabbitMQMetadata.class).orElse(null);
        if (metadata == null || metadata.getHeaders() == null) {
            return null;
        }
        return metadata.getHeader(name, String.class).orElse(null);        
    }
   
   protected void cloudEventsExtensions(Message<byte[]> message, $DataEventType$ event) {

       io.smallrye.reactive.messaging.rabbitmq.IncomingRabbitMQMetadata metadata = message.getMetadata(io.smallrye.reactive.messaging.rabbitmq.IncomingRabbitMQMetadata.class).orElse(null);
        if (metadata == null || metadata.getHeaders() == null) {
            return;
        }
        for (Entry<String, Object> header : metadata.getHeaders().entrySet()) {
            if (header.getKey().startsWith("cloudEvents:")) {
                event.addExtension(header.getKey().replaceFirst("cloudEvents:", ""), header.getValue().toString());
            }
        }
    }
}