...
public class MessageProducer {
	...
	public void produce(ProcessInstance pi, NodeInstance nodeInstance, $Type$ eventData) {
	    ...
	    io.smallrye.reactive.messaging.rabbitmq.OutgoingRabbitMQMetadata metadata = null;
	    if (converter != null && !converter.isUnsatisfied()) {                    
            
            metadata = converter.get().metadata(pi, io.smallrye.reactive.messaging.rabbitmq.OutgoingRabbitMQMetadata.class);
        } 
        if (metadata == null) {
            io.smallrye.reactive.messaging.rabbitmq.OutgoingRabbitMQMetadata.Builder builder = io.smallrye.reactive.messaging.rabbitmq.OutgoingRabbitMQMetadata.builder();
    	    String routingKey = routingKey(pi);
    	    if (routingKey != null) {
    	    ...
    	    if (useCloudEvents.orElse(false)) {
                if (useCloudEventsBinary.orElse(false)) {
                    ...
                    builder.withHeader("cloudEvents:specversion", DataEvent.SPEC_VERSION);
                    builder.withHeader("cloudEvents:id", UUID.randomUUID().toString());
                    builder.withHeader("cloudEvents:source", "/" + pi.getProcessId() + "/" + pi.getId());
                    builder.withHeader("cloudEvents:type", "$TriggerType$");
...
//	    try {
//            boolean success = done.toCompletableFuture().get(5, TimeUnit.SECONDS);
//            if (!success) {
...
//        }
	...
	protected String routingKey(ProcessInstance pi) {
	    return null;
	}
