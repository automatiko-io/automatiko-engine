...
public class MarshallerGenerator {
	...
	public List<CompilationUnit> generate(FileDescriptorSource proto) throws IOException {
		...
		CompilationUnit parsedClazzFile = parse(
				this.getClass().getResourceAsStream("/class-templates/persistence/MessageMarshallerTemplate.java"));

		SerializationContext serializationContext = new SerializationContextImpl(Configuration.builder().build());
		serializationContext
				.registerProtoFiles(FileDescriptorSource.fromResources(classLoader, "automatik-types.proto"));
		...
		for (Entry<String, FileDescriptor> entry : descriptors.entrySet()) {
			...
			if (d.getPackage().equals("io.automatik")) {
				continue;
			}
			...
			for (Descriptor msg : messages) {
				...
				clazz.getImplementedTypes(0)
						.setTypeArguments(NodeList.nodeList(new ClassOrInterfaceType(null, javaType)));

				MethodDeclaration getJavaClassMethod = clazz
						.findFirst(MethodDeclaration.class, md -> md.getNameAsString().equals("getJavaClass"))
				...
				MethodDeclaration getTypeNameMethod = clazz
						.findFirst(MethodDeclaration.class, md -> md.getNameAsString().equals("getTypeName"))
				...
				MethodDeclaration readFromMethod = clazz
						.findFirst(MethodDeclaration.class, md -> md.getNameAsString().equals("readFrom"))
				...
				MethodDeclaration writeToMethod = clazz
						.findFirst(MethodDeclaration.class, md -> md.getNameAsString().equals("writeTo"))
				...
				for (FieldDescriptor field : msg.getFields()) {
					...
					if (protoStreamMethodType != null && !field.isRepeated()) {
						...
						write = new MethodCallExpr(new NameExpr("writer"), "write" + protoStreamMethodType)
								.addArgument(new StringLiteralExpr(field.getName())).addArgument(new MethodCallExpr(
										new NameExpr("t"), accessor + StringUtils.capitalize(field.getName())));
					} else {
						// custom types
						...
						if (field.isRepeated()) {
							...
							write = new MethodCallExpr(new NameExpr("writer"), "writeCollection")
									.addArgument(new StringLiteralExpr(field.getName()))
									.addArgument(new MethodCallExpr(new NameExpr("t"),
					...
					MethodCallExpr setter = new MethodCallExpr(new NameExpr("value"),
							"set" + StringUtils.capitalize(field.getName())).addArgument(read);
					readFromMethod.getBody().ifPresent(b -> b.addStatement(setter));

					// write method
					writeToMethod.getBody()
							.orElseThrow(
									() -> new NoSuchElementException("A method declaration doesn't contain a body!"))
							.addStatement(write);
				}
	...
	protected String javaTypeForMessage(FileDescriptor d, String messageName,
