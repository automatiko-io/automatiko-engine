 ...
 *
 * @see io.automatik.engine.api.workflow.Process
 */
public class ProcessGenerator {
	...
	private MethodDeclaration createInstanceGenericWithWorkflowInstanceMethod(String processInstanceFQCN) {
		...
		VariableDeclarationExpr modelField = new VariableDeclarationExpr(
				new VariableDeclarator().setType(new ClassOrInterfaceType(null, modelTypeName)).setName("model")
						.setInitializer(new MethodCallExpr(new ThisExpr(), CREATE_MODEL)));
	...
	private MethodDeclaration process(ProcessMetaData processMetaData) {
		return processMetaData.getGeneratedClassModel().findFirst(MethodDeclaration.class)
				...
				.setModifiers(Modifier.Keyword.PUBLIC).setType(Process.class.getCanonicalName()).setName("process");
	}
						...
						.setBody(new BlockStmt()
								.addStatement(new MethodCallExpr(new SuperExpr(), "setProcessInstancesFactory")
										.addArgument(new ObjectCreationExpr(null, new ClassOrInterfaceType(null,
												"io.automatik.engine.addons.persistence.ProcessInstancesFactoryImpl"),
												NodeList.nodeList()))));
