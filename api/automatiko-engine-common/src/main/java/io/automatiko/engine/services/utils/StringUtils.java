...
public class StringUtils {
     ...
     */
    public static String padding(final int repeat, final char padChar) throws IndexOutOfBoundsException {
        if (repeat < 0) {
    ...
    /**
     * <p>
     * Unescapes any Java literals found in the <code>String</code>. For example, it
     * will turn a sequence of <code>'\'</code> and <code>'n'</code> into a newline
     * character, unless the <code>'\'</code> is preceded by another
     * <code>'\'</code>.
     * </p>
     * 
    ...
    /**
     * Compares two string being equals ignoring whitespaces, but preserving
     * whitespace between double-quotes The two inputs MUST BE valid DRL/Java syntax
     * (this validation is NOT performed by this method, this method assumes they
     * are). Null check: if either of the input is null, this method will return
     * true IFF both are null. Empty check: if either of the input is an empty
     * string, it will be considered as a whitespace during code-aware comparison.
     */
    public static boolean codeAwareEqualsIgnoreSpaces(String in1, String in2) {
        ...
        int idx1 = 0;
        Character quoted1 = null;
        int idx2 = 0;
        Character quoted2 = null;
        boolean equals = true;
        while (equals) {
            ...
            if (idx1 >= in1.length() || idx2 >= in2.length()) {
                // considered equals if equals check succeeded and both indexes reached end of
                // respective string.
            ...
            switch (string.charAt(charIndex)) {
                ...
                case '"':
                    if (isCurrentCharEscaped || isSingleQuoted) {
                        // ignore escaped double quote and double quote inside single quotes (e.g 'text
                        // " text')
                        continue;
                    }
                ...
                case '\'':
                    if (isCurrentCharEscaped || isDoubleQuoted) {
                        // ignore escaped single quote and single quote inside double quotes (e.g. "text
                        // ' text")
                        continue;
                    }
    ...
    public static boolean isIdentifier(String expr) {
        return !expr.equals("true") && !expr.equals("false") && !expr.equals("null")
                && expr.matches("[\\p{L}_\\$][\\p{L}_\\$\\p{N}]*");
    }
    ...
    public static String toCamelCase(String name) {
        ...
        for (String item : name.split("[\\W_]+")) {
            ...
            if (index == 0) {
                ...
                converted.append(capitalize(item.toLowerCase()));
            }
    ...
    public static String toDashCase(String str) {
        ...
        // Perform the replacement and convert the whole string to lower case
