...
public interface Resource extends Serializable {
	...
	/**
	 * Returns the type of the resource if one could be inferred by the extension of
	 * the resource or if it was explicitly set.
	 *
	...
	/**
	 * Sets this resource name. Some services require a resource to be named and it
	 * is not always possible to infer a name automatically. For instance, a file
	 * resource automatically infers its name from the file and path, but a byte
	 * array resource has to be given a name.
	 *
	...
	/**
	 * In case this resource should be written to a different path when writing it
	 * down to the KieFileSystem, this property allows the application to set such
	 * path.
	 *
	 * @param path the path of the resource
	 *
	...
	/**
	 * Sets the resource type. For some resources it is possible to infer the type
	 * by the file name extension for instance. When it is not possible to infer it
	 * automatically, the user is expected to set it.
	 *
