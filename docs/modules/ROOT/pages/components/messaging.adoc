...
- *mergeMode* - specifies that channel can receive messages from multiple producers. The value of this attribute should be one of following: `MERGE`, `ONE`, `CONCAT`. See more details about merge in the https://smallrye.io/smallrye-reactive-messaging/latest/concepts/merge[reactive messaging documentation].
