    ...
    public ExecutableProcess parseWorkFlow(Reader workflowFile) throws JsonProcessingException {
        ...
        for (State endState : endStates) {
            if (endState.getEnd().getKind() == End.Kind.EVENT) {
                ...
                workflowEndNodes.put(endState.getName(), factory.endNode(idCounter.getAndIncrement(), NODE_END_NAME, false, process));
            }
        }
        ...
        for (State state : workflowStates) {
            if (state.getType().equals(DefaultState.Type.EVENT)) {
                ...
                if (onEventRefs.size() == 1) {
                    ...
                    factory.connect(singleMessageStartNode.getId(), embeddedSubProcess.getId(), singleMessageStartNode.getId() + "_" + embeddedSubProcess.getId(), process, false);
                } else {
                    ...
                    for (String onEventRef : onEventRefs) {
                        ...
                        factory.connect(messageStartNode.getId(), messageStartJoin.getId(), messageStartNode.getId() + "_" + messageStartJoin.getId(), process, false);
                    }
                    factory.connect(messageStartJoin.getId(), embeddedSubProcess.getId(), messageStartJoin.getId() + "_" + embeddedSubProcess.getId(), process, false);
                }

                if (state.getEnd() != null) {
                    if (state.getEnd().isCompensate()) {
                        ActionNode compensationNode = factory.compensationEventNode(idCounter.getAndIncrement(), "Compensation", process, process);
                        factory.connect(embeddedSubProcess.getId(), compensationNode.getId(), embeddedSubProcess.getId() + "_" + compensationNode.getId(), process, false);
                        factory.connect(compensationNode.getId(), workflowEndNodes.get(state.getName()).getId(), compensationNode.getId() + "_" + workflowEndNodes.get(state.getName()).getId(), process, false);
                    } else {
                        factory.connect(embeddedSubProcess.getId(), workflowEndNodes.get(state.getName()).getId(), embeddedSubProcess.getId() + "_" + workflowEndNodes.get(state.getName()).getId(), process, false);
                    }
                }
            ...
            if (state.getType().equals(DefaultState.Type.OPERATION)) {
                ...
                if (state.getStart() != null) {
                    factory.connect(workflowStartNode.getId(), embeddedSubProcess.getId(), workflowStartNode.getId() + "_" + embeddedSubProcess.getId(), process, false);
                }
            ...
            if (state.getType().equals(DefaultState.Type.DELAY)) {
                ...
                if (state.getStart() != null) {
                    factory.connect(workflowStartNode.getId(), timerNode.getId(), workflowStartNode.getId() + "_" + timerNode.getId(), process, false);
                }

                if (state.getEnd() != null) {
                    if (state.getEnd().isCompensate()) {
                        ActionNode compensationNode = factory.compensationEventNode(idCounter.getAndIncrement(), "Compensation", process, process);
                        factory.connect(timerNode.getId(), compensationNode.getId(), timerNode.getId() + "_" + compensationNode.getId(), process, false);
                        factory.connect(compensationNode.getId(), workflowEndNodes.get(state.getName()).getId(), compensationNode.getId() + "_" + workflowEndNodes.get(state.getName()).getId(), process, false);
                    } else {
                        factory.connect(timerNode.getId(), workflowEndNodes.get(state.getName()).getId(), timerNode.getId() + "_" + workflowEndNodes.get(state.getName()).getId(), process, false);
                    }
                }
            ...
            if (state.getType().equals(DefaultState.Type.INJECT)) {
                ...
                if (state.getStart() != null) {
                    factory.connect(workflowStartNode.getId(), actionNode.getId(), workflowStartNode.getId() + "_" + actionNode.getId(), process, false);
                }

                if (state.getEnd() != null) {
                    if (state.getEnd().isCompensate()) {
                        ActionNode compensationNode = factory.compensationEventNode(idCounter.getAndIncrement(), "Compensation", process, process);
                        factory.connect(actionNode.getId(), compensationNode.getId(), actionNode.getId() + "_" + compensationNode.getId(), process, false);
                        factory.connect(compensationNode.getId(), workflowEndNodes.get(state.getName()).getId(), compensationNode.getId() + "_" + workflowEndNodes.get(state.getName()).getId(), process, false);
                    } else {
                        factory.connect(actionNode.getId(), workflowEndNodes.get(state.getName()).getId(), actionNode.getId() + "_" + workflowEndNodes.get(state.getName()).getId(), process, false);
                    }
                }
            ...
            if (state.getType().equals(DefaultState.Type.SUBFLOW)) {
                ...
                if (state.getStart() != null) {
                    factory.connect(workflowStartNode.getId(), callActivityNode.getId(), workflowStartNode.getId() + "_" + callActivityNode.getId(), process, false);
                }

                if (state.getEnd() != null) {
                    if (state.getEnd().isCompensate()) {
                        ActionNode compensationNode = factory.compensationEventNode(idCounter.getAndIncrement(), "Compensation", process, process);
                        factory.connect(callActivityNode.getId(), compensationNode.getId(), callActivityNode.getId() + "_" + compensationNode.getId(), process, false);
                        factory.connect(compensationNode.getId(), workflowEndNodes.get(state.getName()).getId(), compensationNode.getId() + "_" + workflowEndNodes.get(state.getName()).getId(), process, false);
                    } else {
                        factory.connect(callActivityNode.getId(), workflowEndNodes.get(state.getName()).getId(), callActivityNode.getId() + "_" + workflowEndNodes.get(state.getName()).getId(), process, false);
                    }
                }
            ...
            if (state.getType().equals(DefaultState.Type.SWITCH)) {
                ...
                if (switchState.getDataConditions() != null && !switchState.getDataConditions().isEmpty()) {
                    ...
                    if (state.getStart() != null) {
                        factory.connect(workflowStartNode.getId(), splitNode.getId(), workflowStartNode.getId() + "_" + splitNode.getId(), process, false);
                    }
            ...
            if (state.getType().equals(DefaultState.Type.PARALLEL)) {
                ...
                for (Branch branch : parallelState.getBranches()) {
                    ...
                    factory.connect(parallelSplit.getId(), callActivityNode.getId(), parallelSplit.getId() + "_" + callActivityNode.getId(), process, false);
                    factory.connect(callActivityNode.getId(), parallelJoin.getId(), callActivityNode.getId() + "_" + parallelJoin.getId(), process, false);

                }

                if (state.getStart() != null) {
                    factory.connect(workflowStartNode.getId(), parallelSplit.getId(), workflowStartNode.getId() + "_" + parallelSplit.getId(), process, false);
                }

                if (state.getEnd() != null) {
                    if (state.getEnd().isCompensate()) {
                        ActionNode compensationNode = factory.compensationEventNode(idCounter.getAndIncrement(), "Compensation", process, process);
                        factory.connect(parallelJoin.getId(), compensationNode.getId(), parallelJoin.getId() + "_" + compensationNode.getId(), process, false);
                        factory.connect(compensationNode.getId(), workflowEndNodes.get(state.getName()).getId(), compensationNode.getId() + "_" + workflowEndNodes.get(state.getName()).getId(), process, false);
                    } else {
                        factory.connect(parallelJoin.getId(), workflowEndNodes.get(state.getName()).getId(), parallelJoin.getId() + "_" + workflowEndNodes.get(state.getName()).getId(), process, false);
                    }
                }
            ...
            if (transition != null && transition.getNextState() != null) {
                ...
                if (!transition.getProduceEvents().isEmpty()) {
                    if (transition.getProduceEvents().size() == 1) {
                        ...
                        factory.connect(sourceId, sendEventNode.getId(), sourceId + "_" + sendEventNode.getId(), process, false);
                        ...
                        for (ProduceEvent p : transition.getProduceEvents().subList(1, transition.getProduceEvents().size())) {
                            ...
                            if (lastActionNode == null) {
                                ...
                                factory.connect(firstActionNode.getId(), lastActionNode.getId(), firstActionNode.getId() + "_" + lastActionNode.getId(), process, false);
                            } else {
                                factory.connect(lastActionNode.getId(), newActionNode.getId(), lastActionNode.getId() + "_" + newActionNode.getId(), process, false);
                                lastActionNode = newActionNode;
                            }
                        }
                        factory.connect(sourceId, firstActionNode.getId(), sourceId + "_" + firstActionNode.getId(), process, false);
                        factory.connect(lastActionNode.getId(), targetId, lastActionNode + "_" + targetId, process, false);
                    }
                } else {
                    if (transition.isCompensate()) {
                        ActionNode compensationNode = factory.compensationEventNode(idCounter.getAndIncrement(), "Compensation", process, process);
                        factory.connect(sourceId, compensationNode.getId(), sourceId + "_" + compensationNode.getId(), process, false);
                        factory.connect(compensationNode.getId(), targetId, compensationNode.getId() + "_" + targetId, process, false);
                    } else {
        ...
        for (Map.Entry<Long, Error> entry : boundaryIdToError.entrySet()) {
            ...
            if (errorObj.getTransition() != null) {
                ...
                if (errorObj.getTransition().isCompensate()) {
                    ActionNode compensationNode = factory.compensationEventNode(idCounter.getAndIncrement(), "Compensation", process, process);
                    factory.connect(boundaryEventId, compensationNode.getId(), boundaryEventId + "_" + compensationNode.getId(), process, false);
                    factory.connect(compensationNode.getId(), targetId, compensationNode.getId() + "_" + targetId, process, false);
                } else {
            ...
            if (errorObj.getEnd() != null) {
                if (errorObj.getEnd().getKind() == End.Kind.EVENT) {
                    ...
                    if (errorObj.getEnd().isCompensate()) {
                        ActionNode compensationNode = factory.compensationEventNode(idCounter.getAndIncrement(), "Compensation", process, process);
                        factory.connect(boundaryEventId, compensationNode.getId(), boundaryEventId + "_" + compensationNode.getId(), process, false);
                        factory.connect(compensationNode.getId(), boundaryEndNode.getId(), compensationNode.getId() + "_" + boundaryEndNode.getId(), process, false);
                    } else {
                        factory.connect(boundaryEventId, boundaryEndNode.getId(), boundaryEventId + "_" + boundaryEndNode.getId(), process, false);
                    }
                } else {
                    EndNode boundaryEndNode = factory.endNode(idCounter.getAndIncrement(), NODE_END_NAME, false, process);
                    if (errorObj.getEnd().isCompensate()) {
                        ActionNode compensationNode = factory.compensationEventNode(idCounter.getAndIncrement(), "Compensation", process, process);
                        factory.connect(boundaryEventId, compensationNode.getId(), boundaryEventId + "_" + compensationNode, process, false);
                        factory.connect(compensationNode.getId(), boundaryEndNode.getId(), compensationNode.getId() + "_" + boundaryEndNode.getId(), process, false);
                    } else {
                        factory.connect(boundaryEventId, boundaryEndNode.getId(), boundaryEventId + "_" + boundaryEndNode.getId(), process, false);
                    }
            ...
            if (conditions != null && !conditions.isEmpty()) {
                for (EventCondition eventCondition : conditions) {
                    ...
                    if (eventCondition.getTransition() != null) {
                        ...
                        factory.connect(eventSplit.getId(), eventNode.getId(), eventSplit.getId() + "_" + eventNode.getId(), process, false);
                        ...
                        if (eventCondition.getEnd().isCompensate()) {
                            ActionNode compensationNode = factory.compensationEventNode(idCounter.getAndIncrement(), "Compensation", process, process);
                            factory.connect(eventSplit.getId(), eventNode.getId(), eventSplit.getId() + "_" + eventNode.getId(), process, false);
                            factory.connect(eventNode.getId(), compensationNode.getId(), eventNode.getId() + "_" + compensationNode.getId(), process, false);
                            factory.connect(compensationNode.getId(), conditionEndNode.getId(), compensationNode.getId() + "_" + conditionEndNode.getId(), process, false);
                        } else {
                            factory.connect(eventSplit.getId(), eventNode.getId(), eventSplit.getId() + "_" + eventNode.getId(), process, false);
                            factory.connect(eventNode.getId(), conditionEndNode.getId(), eventNode.getId() + "_" + conditionEndNode.getId(), process, false);
                        }
                    }
            ...
            if (switchState.getDefault() != null && switchState.getDefault().getEnd() != null) {
                if (switchState.getDefault().getEnd().getKind() == End.Kind.EVENT) {
                    ...
                    if (switchState.getDefault().getEnd().isCompensate()) {
                        ActionNode compensationNode = factory.compensationEventNode(idCounter.getAndIncrement(), "Compensation", process, process);
                        factory.connect(xorSplit.getId(), compensationNode.getId(), xorSplit.getId() + "_" + compensationNode.getId(), process, false);
                        factory.connect(compensationNode.getId(), defaultEndNode.getId(), compensationNode.getId() + "_" + defaultEndNode.getId(), process, false);
                        xorSplit.getMetaData().put(XORSPLITDEFAULT, xorSplit.getId() + "_" + compensationNode.getId());
                    } else {
                        factory.connect(xorSplit.getId(), defaultEndNode.getId(), xorSplit.getId() + "_" + defaultEndNode.getId(), process, false);
                    ...
                    if (condition.getTransition() != null) {
                        ...
                        if (!condition.getTransition().getProduceEvents().isEmpty()) {
                            ...
                            if (condition.getTransition().getProduceEvents().size() == 1) {
                                ...
                                factory.connect(xorSplit.getId(), sendEventNode.getId(), xorSplit.getId() + "_" + sendEventNode.getId(), process, false);
                                factory.connect(sendEventNode.getId(), nextStateId, sendEventNode.getId() + "_" + nextStateId, process, false);
                                ...
                                long nextStateId = nameToNodeId.get(condition.getTransition().getNextState()).get(NODETOID_START);
                                factory.connect(xorSplit.getId(), firstActionNode.getId(), xorSplit.getId() + "_" + firstActionNode.getId(), process, false);
                                factory.connect(lastActionNode.getId(), nextStateId, lastActionNode.getId() + "_" + nextStateId, process, false);
                        ...
                        if (condition.getEnd().getKind() == End.Kind.EVENT) {
                            ...
                            if (condition.getEnd().isCompensate()) {
                                ...
                                factory.connect(xorSplit.getId(), conditionEndNode.getId(), xorSplit.getId() + "_" + conditionEndNode.getId(), process, false);
        ...
        if (workflowFunctions != null && actions != null && !actions.isEmpty()) {
            ...
            for (Action action : actions) {
                ...
                if (actionFunction != null && actionFunction.getOperation() != null) {
                    ...
                    factory.connect(start.getId(), current.getId(), start.getId() + "_" + current.getId(), embeddedSubProcess, false);
                    start = current;
                } else {
            ...
            try {
                factory.connect(current.getId(), embeddedEndNode.getId(), current.getId() + "_" + embeddedEndNode.getId(), embeddedSubProcess, false);
