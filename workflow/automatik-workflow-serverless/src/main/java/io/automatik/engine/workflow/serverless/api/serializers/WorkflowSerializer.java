...
public class WorkflowSerializer extends StdSerializer<Workflow> {
	...
	public void serialize(Workflow workflow, JsonGenerator gen, SerializerProvider provider) throws IOException {
		...
		if (workflow.getSchemaVersion() != null && !workflow.getSchemaVersion().isEmpty()) {
			gen.writeStringField("schemaVersion", workflow.getSchemaVersion());
		}
		...
		if (workflow.getDataOutputSchema() != null && !workflow.getDataOutputSchema().isEmpty()) {
			gen.writeObjectField("dataOutputSchema", workflow.getDataOutputSchema());
		}
		...
		if (workflow.getExpressionLanguage() != null && !workflow.getExpressionLanguage().isEmpty()) {
			gen.writeObjectField("expressionLanguage", workflow.getExpressionLanguage());
		}
