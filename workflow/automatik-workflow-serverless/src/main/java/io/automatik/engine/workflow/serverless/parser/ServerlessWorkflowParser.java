...
import io.automatik.engine.workflow.serverless.api.actions.Action;
...
import io.automatik.engine.workflow.serverless.api.switchconditions.EventCondition;
...
public class ServerlessWorkflowParser {
	...
	public Process parseWorkFlow(Reader workflowFile) throws JsonProcessingException {
		Workflow workflow = objectMapper.readValue(ServerlessWorkflowUtils.readWorkflowFile(workflowFile),
				Workflow.class);
		...
		if (workflowStartState.getType().equals(Type.EVENT)) {
			...
			workflowStartNode = factory.messageStartNode(idCounter.getAndIncrement(), ServerlessWorkflowUtils
					.getWorkflowEventFor(workflow, startEventState.getEventsActions().get(0).getEventRefs().get(0)),
					process);
		} else {
		...
		for (State endState : endStates) {
			if (endState.getEnd().getKind() == End.Kind.EVENT) {
				workflowEndNodes.put(endState.getName(), factory.messageEndNode(idCounter.getAndIncrement(),
						NODE_END_NAME, workflow, endState.getEnd(), process));
			} else {
		...
		for (State state : workflowStates) {
			if (state.getType().equals(Type.EVENT)) {
				...
				CompositeContextNode embeddedSubProcess = factory.subProcessNode(idCounter.getAndIncrement(),
						state.getName(), process);
				handleActions(workflowFunctions, eventState.getEventsActions().get(0).getActions(), process,
						embeddedSubProcess);
			...
			if (state.getType().equals(Type.SUBFLOW)) {
				...
				SubProcessNode callActivityNode = factory.callActivity(idCounter.getAndIncrement(),
						subflowState.getName(), subflowState.getWorkflowId(), subflowState.isWaitForCompletion(),
						process);
			...
			if (state.getType().equals(Type.SWITCH)) {
				...
				if (switchState.getDataConditions() != null && !switchState.getDataConditions().isEmpty()) {
					...
					Split splitNode = factory.splitNode(idCounter.getAndIncrement(), switchState.getName(),
							Split.TYPE_XOR, process);
					...
					Split splitNode = factory.eventBasedSplit(idCounter.getAndIncrement(), switchState.getName(),
							process);
					if (state.getStart() != null) {
						factory.connect(workflowStartNode.getId(), splitNode.getId(),
								workflowStartNode.getId() + "_" + splitNode.getId(), process);
					...
					LOGGER.warn("unable to determine switch state type (data or event based): {}",
							switchState.getName());
				}
			}
			...
			if (state.getType().equals(Type.PARALLEL)) {
				...
				Split parallelSplit = factory.splitNode(idCounter.getAndIncrement(),
						parallelState.getName() + NODE_START_NAME, Split.TYPE_AND, process);
				Join parallelJoin = factory.joinNode(idCounter.getAndIncrement(),
						parallelState.getName() + NODE_END_NAME, Join.TYPE_AND, process);
				...
				if (transition.getProduceEvent() != null && transition.getProduceEvent().getEventRef() != null) {
					ActionNode sendEventNode = factory.sendEventNode(idCounter.getAndIncrement(),
							ServerlessWorkflowUtils.getWorkflowEventFor(workflow,
									transition.getProduceEvent().getEventRef()),
							process);
		...
		if (eventSplit != null) {
			...
			if (conditions != null && !conditions.isEmpty()) {
				for (EventCondition eventCondition : conditions) {
					...
					EventNode eventNode = factory.consumeEventNode(idCounter.getAndIncrement(), eventDefinition,
							process);

					factory.connect(eventSplit.getId(), eventNode.getId(), eventSplit.getId() + "_" + eventNode,
							process);
		...
		if (xorSplit != null) {
			...
			if (conditions != null && !conditions.isEmpty()) {
				for (DataCondition condition : conditions) {
					...
					if (switchState.getDefault().getNextState() != null && condition.getTransition().getNextState()
							.equals(switchState.getDefault().getNextState())) {
						isDefaultConstraint = true;
					}

					ConstraintImpl constraintImpl = factory.splitConstraint(xorSplit.getId() + "_" + targetId,
							"DROOLS_DEFAULT", "java", ServerlessWorkflowUtils.conditionScript(condition.getPath(),
									condition.getOperator(), condition.getValue()),
							0, isDefaultConstraint);
					xorSplit.addConstraint(
							new ConnectionRef(xorSplit.getId() + "_" + targetId, targetId,
	...
	protected void handleActions(List<Function> workflowFunctions, List<Action> actions, ExecutableProcess process,
			CompositeContextNode embeddedSubProcess) {
		if (actions != null && !actions.isEmpty()) {
			StartNode embeddedStartNode = factory.startNode(idCounter.getAndIncrement(), "EmbeddedStart",
					embeddedSubProcess);
			...
			for (Action action : actions) {
				Function actionFunction = workflowFunctions.stream()
						.filter(wf -> wf.getName().equals(action.getFunctionRef().getRefName())).findFirst().get();

				if (actionFunction.getType() != null) {
					if (SCRIPT_TYPE.equalsIgnoreCase(actionFunction.getType())) {
						String script = ServerlessWorkflowUtils
								.scriptFunctionScript(action.getFunctionRef().getParameters().get(SCRIPT_TYPE_PARAM));
						current = factory.scriptNode(idCounter.getAndIncrement(), action.getFunctionRef().getRefName(),
								script, embeddedSubProcess);
						...
						String script = ServerlessWorkflowUtils
								.sysOutFunctionScript(action.getFunctionRef().getParameters().get(SYSOUT_TYPE_PARAM));
						current = factory.scriptNode(idCounter.getAndIncrement(), action.getFunctionRef().getRefName(),
								script, embeddedSubProcess);
						...
						current = factory.humanTaskNode(idCounter.getAndIncrement(),
								action.getFunctionRef().getRefName(), actionFunction, process, embeddedSubProcess);
						factory.connect(start.getId(), current.getId(), start.getId() + "_" + current.getId(),
								embeddedSubProcess);
						start = current;
					} else if (INTEGRATION_TYPE.equals(actionFunction.getType())) {
						current = factory.camelRouteServiceNode(idCounter.getAndIncrement(),
								action.getFunctionRef().getRefName(), actionFunction, embeddedSubProcess);
						factory.connect(start.getId(), current.getId(), start.getId() + "_" + current.getId(),
								embeddedSubProcess);
						start = current;
					} else {
						LOGGER.warn(
								"currently unsupported function type, supported types are 'script', 'sysout', 'service', 'decision', 'rule', 'integration'");
					...
					LOGGER.warn(
							"invalid function type. supported types are 'script', 'sysout', 'service', 'decision', 'ruleunit'");
					LOGGER.warn("defaulting to script type");
					String script = ServerlessWorkflowUtils.scriptFunctionScript("");
					current = factory.scriptNode(idCounter.getAndIncrement(), action.getFunctionRef().getRefName(),
							script, embeddedSubProcess);
			...
			EndNode embeddedEndNode = factory.endNode(idCounter.getAndIncrement(), "EmbeddedEnd", true,
					embeddedSubProcess);
			try {
				factory.connect(current.getId(), embeddedEndNode.getId(),
						current.getId() + "_" + embeddedEndNode.getId(), embeddedSubProcess);
