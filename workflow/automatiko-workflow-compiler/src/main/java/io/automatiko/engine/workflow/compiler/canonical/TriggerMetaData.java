...
import static io.automatiko.engine.workflow.compiler.canonical.AbstractVisitor.KCONTEXT_VAR;
import static io.automatiko.engine.workflow.process.executable.core.Metadata.MAPPING_VARIABLE;
...
public class TriggerMetaData {
    ...
    public static LambdaExpr buildLambdaExpr(Node node, ProcessMetaData metadata, VariableScope variableScope) {
        ...
        MethodCallExpr producerMethodCall = new MethodCallExpr(new NameExpr("producer_" + node.getId()), "produce")
                .addArgument(new MethodCallExpr(new NameExpr("context"), "getProcessInstance"))
                .addArgument(new MethodCallExpr(new NameExpr("context"), "getNodeInstance"))
