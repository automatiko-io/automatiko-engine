...
import io.automatiko.engine.workflow.compiler.xml.processes.AbstractNodeHandler;
...
public class XmlWorkflowProcessDumper {
	...
	protected void visitHeader(WorkflowProcess process, StringBuilder xmlDump, boolean includeMeta) {
		...
		SwimlaneContext swimlaneContext = (SwimlaneContext) ((io.automatiko.engine.workflow.base.core.Process) process)
				.getDefaultContext(SwimlaneContext.SWIMLANE_SCOPE);
		if (swimlaneContext != null) {
			visitSwimlanes(swimlaneContext.getSwimlanes(), xmlDump);
		}
		ExceptionScope exceptionScope = (ExceptionScope) ((io.automatiko.engine.workflow.base.core.Process) process)
