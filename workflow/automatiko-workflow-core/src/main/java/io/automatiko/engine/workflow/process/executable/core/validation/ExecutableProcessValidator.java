...
package io.automatiko.engine.workflow.process.executable.core.validation;
...
import io.automatiko.engine.workflow.base.core.validation.ProcessValidator;
import io.automatiko.engine.workflow.base.core.validation.impl.ProcessValidationErrorImpl;
...
public class ExecutableProcessValidator implements ProcessValidator {
    ...
    private void checkAllNodesConnectedToStart(final NodeContainer container, boolean isDynamic,
        ...
        if (isDynamic) {
            for (Node node : nodes) {
                if (node.getIncomingConnections(io.automatiko.engine.workflow.process.core.Node.CONNECTION_DEFAULT_TYPE)
                        .isEmpty()) {
