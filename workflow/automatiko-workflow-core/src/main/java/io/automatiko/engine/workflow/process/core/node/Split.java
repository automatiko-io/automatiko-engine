...
public class Split extends NodeImpl implements Constrainable {
	...
	/**
	 * All outgoing connections of a split of this type are triggered when its
	 * incoming connection has been triggered. A split of this type should have no
	 * constraints linked to any of its outgoing connections.
	 */
	public static final int TYPE_AND = 1;
	/**
	 * Exactly one outgoing connection of a split of this type is triggered when its
	 * incoming connection has been triggered. Which connection is based on the
	 * constraints associated with each of the connections: the connection with the
	 * highest priority whose constraint is satisfied is triggered.
	 */
	public static final int TYPE_XOR = 2;
	/**
	 * One or multiple outgoing connections of a split of this type are triggered
	 * when its incoming connection has been triggered. Which connections is based
	 * on the constraints associated with each of the connections: all connections
	 * whose constraint is satisfied are triggered.
	 */
...
//    private Map<ConnectionRef, Constraint> constraints = new HashMap<ConnectionRef, Constraint>();
	...
	public boolean isDefault(final Connection connection) {
		...
		throw new UnsupportedOperationException(
				"Constraints are " + "only supported with XOR or OR split types, not with: " + getType());
	}
	...
	public void setConstraint(final Connection connection, final Constraint constraint) {
		if (this.type == TYPE_OR || this.type == TYPE_XOR) {
			...
			throw new UnsupportedOperationException(
					"Constraints are " + "only supported with XOR or OR split types, not with type:" + getType());
		}
	}
	...
	public void validateAddIncomingConnection(final String type, final Connection connection) {
		...
		if (!getIncomingConnections(io.automatiko.engine.workflow.process.core.Node.CONNECTION_DEFAULT_TYPE).isEmpty()) {
			throw new IllegalArgumentException("This type of node [" + connection.getTo().getMetaData().get("UniqueId")
