...
public class CompositeNode extends StateBasedNode implements NodeContainer, EventNodeInterface {
	...
	public void addNode(Node node) {
		...
		if (node.getId() <= 0) {
			...
			((io.automatiko.engine.workflow.process.core.Node) node).setId(++id);
		}
		...
		if (inNode != null) {
			...
			for (Connection connection : connections) {
				...
				if (inNode.getNode() != null) {
					new ConnectionImpl(start, io.automatiko.engine.workflow.process.core.Node.CONNECTION_DEFAULT_TYPE,
	...
	public void addOutgoingConnection(String type, Connection connection) {
		if (connection.getTo().getParentContainer() == this) {
			linkIncomingConnections(io.automatiko.engine.workflow.process.core.Node.CONNECTION_DEFAULT_TYPE,
