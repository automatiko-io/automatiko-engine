...
public final class Sig<T> implements Signal<T> {
    ...
    public static <T> io.automatiko.engine.api.workflow.Signal<T> of(String channel, T payload) {
        return new Sig<>(channel, payload);
    }

    public static <T> io.automatiko.engine.api.workflow.Signal<T> of(String channel, T payload, String referenceId) {
        return new Sig<>(channel, payload, referenceId);
    }

    public static <T> io.automatiko.engine.api.workflow.Signal<T> of(String channel) {
