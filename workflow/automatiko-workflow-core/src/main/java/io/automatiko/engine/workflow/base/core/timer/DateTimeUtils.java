...
public class DateTimeUtils extends TimeUtils {
    ...
    public static long[] parseRepeatableDateTime(String dateTimeStr) {
        ...
        if (isRepeatable(Objects.requireNonNull(dateTimeStr, "Date-time string cannot be a null value!"))) {
            ...
            if (DateTimeUtils.isPeriod(delayIn) && DateTimeUtils.isPeriod(periodIn)) {
                // If delay and period is specified as duration then us it directly                
                period = Duration.parse(periodIn);
                startAtDelayDur = Duration.parse(delayIn);
            } else if (DateTimeUtils.isPeriod(delayIn)) {
                // If delay is specified as duration then period variable carry end time
                // information
                ...
                // If period is specified as duration then delay variable carry start time
                // information
