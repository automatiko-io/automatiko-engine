...
package io.automatiko.engine.workflow.base.core.transformation;
...
/**
 * Implementation of <code>DataTransformer</code> that is based on standard Java
 * scripting capabilities (javax.script). By default it uses Rhino scripting
 * engine for JavaScript evaluation. But supports all scripting engine that are
 * compliant with JSR 223. It's just a matter of placing them on classpath so
 * Java itself can discover it and then new instance of this class will be
 * registered for that engine. <br/>
 * Allows to pass custom properties to the engine via property file that should
 * be placed on root of the classpath named 'FQCN of the script engine
 * factory'.properties <br/>
 ...
 * <li>result of the output produced by the script engine - will be used only
 * when eval returns null</li>
 * </ul>
 */
public class JavaScriptingDataTransformer implements DataTransformer {
	...
	protected Object evaluateExpression(Object expression, Map<String, Object> parameters) {
		try {
			...
			if (expression instanceof CompiledScript) {
				logger.debug("About to evaluate compiled expression {} with bindings {} on engine", expression,
	...
	protected void registerAttributes() {
		try {
			InputStream propsIn = this.getClass()
					.getResourceAsStream("/" + this.factory.getClass().getName() + ".properties");
			if (propsIn != null) {
