...
public class BPMNPlaneHandler extends BaseAbstractHandler implements Handler {
	...
	private boolean processNodeInfo(NodeInfo nodeInfo, Node[] nodes) {
		...
		for (Node node : nodes) {
			...
			if (nodeInfo.getNodeRef().equals(id)) {
				((io.automatik.engine.workflow.process.core.Node) node).setMetaData("x", nodeInfo.getX());
				((io.automatik.engine.workflow.process.core.Node) node).setMetaData("y", nodeInfo.getY());
				((io.automatik.engine.workflow.process.core.Node) node).setMetaData("width", nodeInfo.getWidth());
				((io.automatik.engine.workflow.process.core.Node) node).setMetaData("height", nodeInfo.getHeight());
				return true;
			}
	...
	private void postProcessNodeOffset(Node[] nodes, int xOffset, int yOffset) {
		for (Node node : nodes) {
			...
			if (x != null) {
				((io.automatik.engine.workflow.process.core.Node) node).setMetaData("x", x - xOffset);
			}
			Integer y = (Integer) node.getMetaData().get("y");
			if (y != null) {
				((io.automatik.engine.workflow.process.core.Node) node).setMetaData("y", y - yOffset);
			}
			if (node instanceof NodeContainer) {
