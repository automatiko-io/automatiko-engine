...
public abstract class AbstractNodeHandler extends BaseAbstractHandler implements Handler {
	...
	public Object start(final String uri, final String localName, final Attributes attrs,
		...
		if ("true".equalsIgnoreCase(System.getProperty("jbpm.v5.id.strategy"))) {
			try {
				...
				for (io.automatik.engine.api.definition.process.Node n : nodeContainer.getNodes()) {
					if (n.getId() > newId) {
						newId = n.getId();
					}
				}
				((io.automatik.engine.workflow.process.core.Node) node).setId(++newId);
			}
		} else {
	...
	protected String getErrorIdForErrorCode(String errorCode, Node node) {
		io.automatik.engine.api.definition.process.NodeContainer parent = node.getParentContainer();
