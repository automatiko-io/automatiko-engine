...
import io.automatik.engine.workflow.compiler.xml.processes.AbstractNodeHandler;
...
public class XmlWorkflowProcessDumper {
	...
	public XmlWorkflowProcessDumper(String type, String namespace, String schemaLocation,
	...
	protected void visitProcess(WorkflowProcess process, StringBuilder xmlDump, boolean includeMeta) {
		xmlDump.append("<?xml version=\"1.0\" encoding=\"UTF-8\"?> " + EOL + "<process xmlns=\"" + namespace + "\""
				+ EOL + "         xmlns:xs=\"http://www.w3.org/2001/XMLSchema-instance\"" + EOL
				+ "         xs:schemaLocation=\"" + namespace + " " + schemaLocation + "\"" + EOL + "         type=\""
				+ type + "\" ");
	...
	protected void visitHeader(WorkflowProcess process, StringBuilder xmlDump, boolean includeMeta) {
		...
		visitImports(((io.automatik.engine.workflow.base.core.Process) process).getImports(), xmlDump);
		visitGlobals(((io.automatik.engine.workflow.base.core.Process) process).getGlobals(), xmlDump);
		visitFunctionImports(((io.automatik.engine.workflow.base.core.Process) process).getFunctionImports(), xmlDump);
		...
		SwimlaneContext swimlaneContext = (SwimlaneContext) ((io.automatik.engine.workflow.base.core.Process) process)
				.getDefaultContext(SwimlaneContext.SWIMLANE_SCOPE);
		if (swimlaneContext != null) {
			visitSwimlanes(swimlaneContext.getSwimlanes(), xmlDump);
		}
		ExceptionScope exceptionScope = (ExceptionScope) ((io.automatik.engine.workflow.base.core.Process) process)
	...
	private void visitGlobals(Map<String, String> globals, StringBuilder xmlDump) {
		if (globals != null && globals.size() > 0) {
			...
			for (Map.Entry<String, String> global : globals.entrySet()) {
				xmlDump.append("      <global identifier=\"" + global.getKey() + "\" type=\"" + global.getValue()
						+ "\" />" + EOL);
			}
			xmlDump.append("    </globals>" + EOL);
		}
	...
	public static void visitValue(Object value, DataType dataType, StringBuilder xmlDump) {
		xmlDump.append(
				"        <value>" + XmlDumper.replaceIllegalChars(dataType.writeValue(value)) + "</value>" + EOL);
	}
	...
	public void visitNode(Node node, StringBuilder xmlDump, boolean includeMeta) {
		...
		if (handler != null) {
			((AbstractNodeHandler) handler).writeNode((io.automatik.engine.workflow.process.core.Node) node, xmlDump,
					includeMeta);
		} else {
