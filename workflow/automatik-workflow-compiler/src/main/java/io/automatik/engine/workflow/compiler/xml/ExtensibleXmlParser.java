...
public class ExtensibleXmlParser extends DefaultHandler {
	 ...
	 */
	public Object read(final Reader reader) throws SAXException, IOException {
	 ...
	 * @throws ParserConfigurationException
	 */
	public Object read(final InputStream inputStream) throws SAXException, IOException {
	...
	public Object read(final InputSource in) throws SAXException, IOException {
		if (this.docFragment == null) {
			...
			try {
				...
				// obscure JDK1.5 bug where FactoryFinder in the JRE returns a null ClassLoader,
				// so fall back to hard coded xerces.
				// https://stg.network.org/bugzilla/show_bug.cgi?id=47169
				// http://bugs.sun.com/bugdatabase/view_bug.do?bug_id=4633368
				try {
					f = (DocumentBuilderFactory) Class.forName("org.apache.xerces.jaxp.DocumentBuilderFactoryImpl")
							.newInstance();
				} catch (Exception e1) {
		...
		if (this.parser == null) {
			...
			try {
				...
				// http://bugs.sun.com/bugdatabase/view_bug.do?bug_id=4633368
				try {
					factory = (SAXParserFactory) Class.forName("org.apache.xerces.jaxp.SAXParserFactoryImpl")
							.newInstance();
				} catch (Exception e1) {
					throw new RuntimeException("Unable to create new DOM Document", e1);
				}
	...
	private void validate(final String uri, final String localName, final Handler handler) throws SAXParseException {
	 ...
	 */
	public void startElementBuilder(final String tagName, final Attributes attrs) {
	...
	Handler getHandler(final String uri, final String localName) {
	...
	protected String buildPrintMessage(final SAXParseException x) {
		return this.message
				.format(new Object[] { x.getSystemId(), x.getLineNumber(), x.getColumnNumber(), x.getMessage() });
	}
	...
	private InputSource resolveSchema(final String publicId, final String systemId) throws SAXException, IOException {
	...
	/**
	 * Initializes EntityResolver that is configured via system property
