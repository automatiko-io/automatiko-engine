...
public abstract class AbstractNodeHandler extends BaseAbstractHandler implements Handler {
	...
	protected void handleNode(final Node node, final Element element, final String uri, final String localName,
		...
		if (color != null && color.length() != 0) {
			try {
				...
				throw new SAXParseException("<" + localName + "> requires an Integer 'color' attribute",
						parser.getLocator());
			}
		}
	}
	...
	protected void writeNode(final String name, final Node node, final StringBuilder xmlDump,
	...
	protected boolean containsMetaData(final Node node) {
		for (Map.Entry<String, Object> entry : ((NodeImpl) node).getMetaData().entrySet()) {
			...
			if (!"x".equals(name) && !"y".equals(name) && !"width".equals(name) && !"height".equals(name)
					&& !"color".equals(name) && !"UniqueId".equals(name) && entry.getValue() instanceof String) {
				return true;
			}
		}
		return false;
	...
	protected void writeMetaData(final Node node, final StringBuilder xmlDump) {
		for (Map.Entry<String, Object> entry : ((NodeImpl) node).getMetaData().entrySet()) {
			String name = entry.getKey();
			if (!"x".equals(name) && !"y".equals(name) && !"width".equals(name) && !"height".equals(name)
					&& !"color".equals(name) && entry.getValue() instanceof String) {
	...
	public static void writeAction(final ProcessAction action, final StringBuilder xmlDump) {
