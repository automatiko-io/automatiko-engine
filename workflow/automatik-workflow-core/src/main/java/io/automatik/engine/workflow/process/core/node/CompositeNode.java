...
import io.automatik.engine.workflow.process.core.impl.NodeContainerImpl;
...
public class CompositeNode extends StateBasedNode implements NodeContainer, EventNodeInterface {
	...
	public void addNode(Node node) {
		...
		if (node.getId() <= 0) {
			...
			((io.automatik.engine.workflow.process.core.Node) node).setId(++id);
		}
	...
	public void linkIncomingConnections(String inType, CompositeNode.NodeAndType inNode) {
		...
		if (oldNodeAndType != null) {
			if (oldNodeAndType.equals(inNode)) {
				...
				List<Connection> oldInConnections = oldNodeAndType.getNode()
						.getIncomingConnections(oldNodeAndType.getType());
		...
		if (inNode != null) {
			...
			for (Connection connection : connections) {
				...
				if (inNode.getNode() != null) {
					new ConnectionImpl(start, io.automatik.engine.workflow.process.core.Node.CONNECTION_DEFAULT_TYPE,
	...
	public void linkOutgoingConnections(CompositeNode.NodeAndType outNode, String outType) {
		...
		if (oldNodeAndType != null) {
			if (oldNodeAndType.equals(outNode)) {
				...
				List<Connection> oldOutConnections = oldNodeAndType.getNode()
						.getOutgoingConnections(oldNodeAndType.getType());
		...
		if (outNode != null) {
			...
			for (Connection connection : connections) {
				...
				if (outNode.getNode() != null) {
					new ConnectionImpl(outNode.getNode(), outNode.getType(), end,
	...
	public void addIncomingConnection(String type, Connection connection) {
		if (connection.getFrom().getParentContainer() == this) {
			linkOutgoingConnections(connection.getFrom().getId(), connection.getFromType(),
					io.automatik.engine.workflow.process.core.Node.CONNECTION_DEFAULT_TYPE);
		} else {
	...
	public void addOutgoingConnection(String type, Connection connection) {
		if (connection.getTo().getParentContainer() == this) {
			linkIncomingConnections(io.automatik.engine.workflow.process.core.Node.CONNECTION_DEFAULT_TYPE,
		...
		if (nodeAndType != null) {
			for (Connection inConnection : nodeAndType.getNode().getIncomingConnections(nodeAndType.getType())) {
				if (((CompositeNodeStart) inConnection.getFrom()).getInNodeId() == connection.getFrom().getId()) {
					((NodeImpl) nodeAndType.getNode()).validateRemoveIncomingConnection(nodeAndType.getType(),
							inConnection);
					return;
				}
			}
		...
		if (nodeAndType != null) {
			for (Connection outConnection : nodeAndType.getNode().getOutgoingConnections(nodeAndType.getType())) {
				if (((CompositeNodeEnd) outConnection.getTo()).getOutNodeId() == connection.getTo().getId()) {
					((NodeImpl) nodeAndType.getNode()).validateRemoveOutgoingConnection(nodeAndType.getType(),
							outConnection);
					return;
				}
			}
	...
	public static class NodeAndType implements Serializable {
		...
		public boolean equals(Object o) {
			if (o instanceof NodeAndType) {
				return nodeId == ((NodeAndType) o).nodeId && type.equals(((NodeAndType) o).type);
			}
			return false;
		}
