...
public class JoinInstance extends NodeInstanceImpl {
	...
	private boolean existsActiveDirectFlow(NodeInstanceContainer nodeInstanceContainer, final Node lookFor) {
		...
		for (NodeInstance nodeInstance : activeNodeInstances) {
			...
			// black box
			if (((io.automatik.engine.workflow.process.instance.NodeInstance) nodeInstance).getLevel() != getLevel()) {
				continue;
			}
	...
	public void triggerCompleted() {
		...
		// state
		triggerCompleted(io.automatik.engine.workflow.process.core.Node.CONNECTION_DEFAULT_TYPE, triggers.isEmpty());
	}
