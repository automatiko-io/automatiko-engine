...
public class SystemMetricsCollectorTest {
	...
	public void givenAnExceptionWhenRegisterExceptionIsCalledThenTheStatusCodeIsExportedToPrometheus() {
		...
		// Assert
		assertEquals(mapExceptionRepetition.get("NoSuchElement"),
				getLabelsValue("api_http_stacktrace_exceptions", handler, "NoSuchElement").intValue());
		assertEquals(mapExceptionRepetition.get("Exception"),
				getLabelsValue("api_http_stacktrace_exceptions", handler, "Exception").intValue());
	}
	...
	private double getQuantile(String name, String labelValue, double q) {
		return registry.getSampleValue(name, new String[] { "endpoint", "quantile" },
				new String[] { labelValue, Collector.doubleToGoString(q) }).doubleValue();
	}

	private Double getLabelsValue(String name, String handler, String identifier) {
		return registry.getSampleValue(name, new String[] { "endpoint", "identifier" },
				new String[] { handler, identifier });
	}
}
