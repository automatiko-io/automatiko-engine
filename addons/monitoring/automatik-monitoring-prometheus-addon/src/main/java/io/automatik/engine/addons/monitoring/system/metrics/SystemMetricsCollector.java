...
package io.automatik.engine.addons.monitoring.system.metrics;
...
public class SystemMetricsCollector {
	...
	private static final Counter REQUEST_STATUS_CODE_COUNTER = Counter.build().name(STATUS_CODE_NAME)
			.help(STATUS_CODE_HELP).labelNames(ENDPOINT_INDENTIFIER_LABELS).register();

	private static final Counter EXCEPTIONS_COUNTER = Counter.build().name(EXCEPTIONS_NAME).help(EXCEPTIONS_HELP)
			.labelNames(ENDPOINT_INDENTIFIER_LABELS).register();

	private static final Summary ELAPSED_TIME_SUMMARY = Summary.build() // Calculate quantiles over a sliding window of
																		// 3 minutes.
			.quantile(0.1, 0.01) // Add 10th percentile with 5% tolerated error
			.quantile(0.25, 0.05).quantile(0.50, 0.05).quantile(0.75, 0.05).quantile(0.9, 0.05).quantile(0.99, 0.01)
			.maxAgeSeconds(180).name(ELAPSED_TIME_NAME).help(ELAPSED_TIME_HELP).labelNames(ENDPOINT_LABEL).register();
